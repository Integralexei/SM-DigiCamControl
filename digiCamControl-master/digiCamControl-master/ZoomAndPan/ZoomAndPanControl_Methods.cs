...
namespace ZoomAndPan
{
    public partial class ZoomAndPanControl
    {
        ...
        public void ScaleToFit()
        {
            ...
            // Skip transient/placeholder content (e.g. 30×30 margin-only state during image transitions).
            // Real photos are thousands of pixels; anything < 100px is not the final content.
            if (_unScaledExtent.Width < 100 || _unScaledExtent.Height < 100 || ActualWidth < 1 || ActualHeight < 1)
            {
                _fitRequested = true;
                return;
            }
            ...
            _fitRequested = false;
            var fitZoom = Math.Min(ActualWidth / _unScaledExtent.Width, ActualHeight / _unScaledExtent.Height);

            // Cap at 1.0: "Fit" means shrink-to-fit, never enlarge beyond 100%.
            // This prevents thumbnails (542×371) from being zoomed in to 190%.
            fitZoom = Math.Min(fitZoom, 1.0);

            var clampedZoom = Math.Min(Math.Max(fitZoom, MinimumZoomClamped), MaximumZoom);
            InternalViewportZoom = clampedZoom;
            ContentOffsetX = 0;
            ContentOffsetY = 0;
        }
